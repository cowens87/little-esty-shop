...
describe 'As a merchant' do
  describe "When I visit my merchant's invoice show page '/merchants/merchant_id/invoices/invoice_id'" do
    before :each do
      ...
      # Discounts:
      @discount_1 = Discount.create!(discount_percentage: 10, quantity_threshold: 5, merchant_id: @amazon.id)
    ...
    describe 'Merchant Invoice Show Page: Total Revenue includes discounts' do
      it 'I see that the total revenue for my merchant includes bulk discounts in the calculation' do
        ...
        expect(page).to have_content('Total Revenue: $1450.0')
        expect(page).to have_content('Discount Applied: -$237.5')
        expect(page).to have_content('Total After Discount: $1212.5')
      end
    end
